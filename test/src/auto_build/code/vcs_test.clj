(ns auto-build.code.vcs-test
  (:require
   ...
   [clojure.test :refer [#_is deftest testing]]))

(deftest latest-commit-message-test
  #_(is (= {:status :success
            :git-describe {:status :success}
            :tag "wip"}
           (-> (sut/latest-commit-message {:normalln println
                                           :errorln println}
                                          ""
                                          false)
...
(deftest latest-commit-sha-test
  #_(is (= {:status :success
           ...
           (-> (sut/latest-commit-sha {:normalln println
                                       :errorln println}
                                      ""
                                      false)
               (update :git-describe select-keys [:status])))))
...
(deftest current-branch-test
  #_(is (= {:status :success
           ...
           (-> (sut/current-branch {:normalln println
                                    :errorln println}
                                   ""
                                   false)
...
(deftest clean-state-test
  (testing
    ...
    #_(with-out-str (testing "In a dirty status"
                      (is (= {:status :dirty-state
                              :git-status {:status :success}}
                             (-> (sut/clean-state {:normalln println
                                                   :errorln println}
                                                  "."
                                                  true)
    ...
    #_(testing "In a clean status"
        (is (= :success
               (:status (sut/clean-state {:normalln println
                                          :errorln println}
                                         "."
                                         true)))
            "When the status")
...
(deftest nothing-to-push-test
  (testing
    "That tests are removed as they should be executed on dev environement and are dependant on the state"
    #_(with-out-str (testing "In a dirty status"
                      (is (= {:status :not-pushed
                              :git-status {:status :success}}
                             (-> (sut/nothing-to-push {:normalln println
                                                       :errorln println}
                                                      "."
                                                      true)
                                 (update :git-status select-keys [:status])))
...
(deftest current-tag-test
  #_(is (= {:status :cmd-failed
            :git-describe {:status :fail
                           :out-stream []
                           :err-stream ["fatal: No names found, cannot describe anything."]}}
           (-> (sut/current-tag {:errorln println
                                 :normalln println}
                                ""
                                false)
               (update :git-describe select-keys [:status :out-stream :err-stream])))
        "A brand new repo"))

(deftest current-repo-url-test
  #_(is (= {:status :success
            :git-remote {:status :success
                         :out-stream ["origin\tgit@github.com:hephaistox/auto-build.git (push)"
                                      "origin\tgit@github.com:hephaistox/auto-build.git (fetch)"]
                         :err-stream []}
            :repo-url "git@github.com:hephaistox/auto-build.git"}
           (-> (sut/current-repo-url {:errorln println
                                      :normalln println}
                                     ""
                                     false)
...
(deftest gh-run-wip-test
  #_(is (= {:status :success
            ...
            :last-run {:run-id true
                       :status :run-ok}}
           (-> (sut/gh-run-wip? {:normalln println
                                 :errorln println}
                                ""
                                true)
...
(deftest gh-run-view-test
  #_(is
     (=
      ...
      (-> (sut/gh-run-view {:normalln println
                            :errorln println}
                           ""
                           true
                           12776650208)
