(ns auto-build.code.doc
  (:require [auto-build.os.cmd :as build-cmd]
            [auto-build.os.file :as build-file]
            [clojure.string :as str]
            [auto-build.os.filename :as build-filename]))

(defn build-version
  [{:keys [normalln errorln uri-str], :as _printers} app-dir version doc-dir
   verbose]
  (let [cmd ["clojure" "-X:codox" ":version" (build-cmd/parameterize version)
             :output-path (build-cmd/parameterize doc-dir)]]
    (if verbose
...
(defn copy-version
  [{:keys [normalln errorln], :as _printers} app-dir version doc-dir verbose]
...
(defn link-to-latest
  [{:keys [normalln errorln], :as _printers} app-dir version verbose]
  (let [version-dir (build-filename/create-dir-path app-dir version)
...
(defn commit-changes
  [{:keys [normalln], :as _printers} app-dir version verbose]
  (let [cmd ["git" "commit" "-m" version]
        _ (when verbose (normalln "Execute" cmd))
        res (-> cmd
                (build-cmd/as-string app-dir))
        {:keys [status out-stream]} res]
    {:status (if (or (= :success status)
                     (str/includes? out-stream "nothing to commit"))
               :success
               :commit-failed),
     :commit-res res}))
